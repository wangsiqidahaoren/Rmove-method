...
import pandas as pd
from interactive_predict import base_path
import reader
...
class Model:
    ...
    def path_rnn_last_state(self, is_evaluating, path_embed, path_lengths, valid_contexts_mask):
        ...
        # print("+++++++++++++++++++++++++++++++++++++++")
        # print(self.config.MAX_PATH_LENGTH)
    ...
    def compute_contexts(self, subtoken_vocab, nodes_vocab, source_input, nodes_input,
        ...
        self.ans_Vector.append(batched_embed)
    ...
    def predict(self, predict_data_lines, input_filename):
        ...
        for line in predict_data_lines:
            ...
            # attention_average = np.empty((1,200))
            # for i in range(len(attention_weights)):
            #     attention_average += attention_weights[i]/6

            # result_pd = pd.DataFrame(attention_average)
            # result_pd.to_csv('attention.csv', encoding='utf-8')
            # attention_average = (attention_weights[0]+attention_weights[1]+attention_weights[2]+attention_weights[3]+attention_weights[4]+attention_weights[5])/6

            # print(attention_average)

            # print(len(self.ans_Vector))
            # print(self.ans_Vector[0])
            with self.sess.as_default():
                for ans_v in self.ans_Vector:
                    a = self.sess.run(ans_v, feed_dict={self.predict_placeholder: line})
                    # print(type(a[0]))
                    # print(len(a[0]))
                    # print(a)
                    code_vec = (1 / self.config.MAX_TARGET_PARTS) * np.matmul(attention_weights.sum(axis=0), a[0])
                    # print(code_vec)
                name = base_path + '_vector.txt'
                with open(name, 'a') as file:
                    MethodName = input_filename + "_" + line.split(' ')[0].title()
                    code_vec_str = [str(x) for x in code_vec.tolist()]
                    file.write(MethodName.replace('|', '') + " ")
                    file.write(' '.join(code_vec_str))
                    file.write("\n")
                #
                # result_pd = pd.DataFrame(a[0])
                # result_pd.to_csv('vector.csv', encoding='utf-8')

                # multi_res = np.dot(attention_average, a[0])
                # result_pd = pd.DataFrame(multi_res)
                # result_pd.to_csv('multi_res.csv', encoding='utf-8')
            # print("---------------------------------------------------------------------------------------")
