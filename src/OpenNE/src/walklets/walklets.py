...
class WalkletMachine:
    ...
    def get_embedding(self, model):
        ...
        """
        embedding = []
        # for node in range(len(selft.graph.nodes())):
    ...
    def create_embedding(self):
        ...
        self.embedding=PCA(n_components=128).fit_transform(self.embedding)
    ...
    def save_model(self):
        ...
        self.row_names=[str(x) for x in self.graph.nodes()]
        self.embedding = pd.DataFrame(self.embedding,index=self.row_names,columns=self.column_names)
        self.embedding.to_csv(self.args.output)
