...
# encoding: utf-8
"""
@description: 用LR训练并对JMove数据进行测试
@date: 2021/12/18
"""
...
def train(X_train, X_test, y_train, y_test, trainingfile):
    ...
    # 网格搜索参数列表
    tuned_parameters = {
        'C':np.append(np.arange(0.01,1,0.01),[1,10,100]),
        'max_iter':range(10,101,30),
        'tol':[0.0001,0.001,0.01,0.1],}
    # 生成模型
    print("Start trainging : " + trainingfile + "\n")
    grid = GridSearchCV(LogisticRegression(penalty='l2',solver='liblinear'),tuned_parameters,cv=5,scoring='roc_auc', verbose=2)
    grid.fit(X_train, y_train)
    cls = grid.best_estimator_
    info = str(grid.best_params_)
    # 把数据交给模型训练
